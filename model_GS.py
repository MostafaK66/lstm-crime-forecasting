...
class DeepLearningModelGS:
    ...
    def lstm_enc_dec(self, param):
        ...
        enc = LSTM(param['lstm_unit'], return_sequences=True, activation=param["activation_function"], dropout=param["dropout_ratio"])(enc)
        enc = LSTM(param['lstm_unit'], return_sequences=True, activation=param["activation_function"], dropout=param["dropout_ratio"])(enc)
        enc = LSTM(param['lstm_unit'], return_sequences=False, activation=param["activation_function"], dropout=param["dropout_ratio"])(enc)
        ...
        dec = LSTM(param['lstm_unit'], return_sequences=True, activation=param["activation_function"], dropout=param["dropout_ratio"])(dec)
        dec = LSTM(param['lstm_unit'], return_sequences=True, activation=param["activation_function"], dropout=param["dropout_ratio"])(dec)
        dec = LSTM(param['lstm_unit'], return_sequences=True, activation=param["activation_function"], dropout=param["dropout_ratio"])(dec)
    ...
    def perform_hyperparameter_tuning(self, X_train, y_train, X_valid, y_valid):
        ...
        kgs = KerasGridSearch(hypermodel, self.param_grid, monitor='val_loss', greater_is_better=False, tuner_verbose=2)
    ...
    def save_best_params(self, best_params, best_score, trained_path, total_duration, parameter_tuning="GS"):
